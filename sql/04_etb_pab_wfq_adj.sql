SELECT 7 AS Stale_Suppression_Days, /* Days past due for stale suppression*/ 7 AS Fence_Suppression_Days, /* Near-term fence window (days forward)*/ 45 AS WC_Inventory_Age_Days, /* Max age of active WC bin inventory*/ 90 AS Cycle_Count_Overdue_Days, 
             /* Days before cycle count is OVERDUE*/ 7 AS Early_Issue_Flag_Days/* Early issue detection threshold*/ ), /* ============================================================================*/ p_norm AS
...
FROM   dbo.ETB_ActiveDemand_Union_FG_MO m), /* ============================================================================*/ item_desc AS
    ...
    (SELECT CAST(COALESCE (NULLIF (LTRIM(RTRIM(p_norm.ORDERNUMBER)), ''), NULLIF (LTRIM(RTRIM(m_norm.ORDERNUMBER)), '')) AS varchar(255)) AS ORDERNUMBER, CAST(ISNULL(m_norm.Customer, '') AS varchar(255)) AS Construct, CAST(ISNULL(m_norm.FG, '') AS varchar(255)) 
                 ...
                 AS varchar(50)) AS UOMSCHDL, p_norm.STSDESCR, p_norm.DUEDATE, p_norm.[Expiry Dates], p_norm.[Date + Expiry], p_norm.MRPTYPE, p_norm.VendorItem, /* Issue 4: UNASSIGNED fallback + source tracking*/ COALESCE (NULLIF (p_norm.PRIME_VNDR, ''), 'UNASSIGNED') 
                 AS PRIME_VNDR, CASE WHEN p_norm.PRIME_VNDR IS NOT NULL AND LTRIM(RTRIM(p_norm.PRIME_VNDR)) <> '' THEN 'PAB_MO' ELSE 'UNASSIGNED' END AS Vendor_Data_Source, p_norm.PURCHASING_LT, p_norm.PLANNING_LT, p_norm.ORDER_POINT_QTY, 
                 p_norm.SAFETY_STOCK, p_norm.Deductions AS Original_Deductions, p_norm.Expiry AS Original_Expiry, p_norm.[PO's] AS Original_POs, p_norm.Running_Balance AS Original_Running_Balance, 
...
WHERE EXISTS
    ...
    (SELECT Core.*, ISNULL(ml.MRP_IssueDate, '') AS MRP_IssueDate, ISNULL(ml.WCID_I, '') AS WCID_From_MO, ISNULL(ml.Total_Issued, 0) AS Issued, ISNULL(ml.Remaining_Required, 0) AS Remaining, CASE WHEN ISNULL(ml.Total_Issued, 0) > 0 THEN 'YES' ELSE 'NO' END AS Has_Issued, 
                 ...
                 Core.[Date + Expiry], ' - ', ml.Required_Qty - ml.Total_Issued) END AS Unified_Value, /* Issue 2: WC site pattern validation*/ CASE WHEN ISNULL(ml.WCID_I, '') 
                 = '' THEN 'NO_WC' WHEN ml.WCID_I LIKE 'WC-W%' THEN 'VALID_WC' ELSE 'NON_WC_SITE' END AS WC_Site_Validation
    FROM    Core LEFT JOIN
...
/* ============================================================================*/ Base AS
    ...
    (SELECT CAST(NULL AS varchar(31)) AS ITEMNMBR, CAST(NULL AS date) AS Last_Cycle_Count_Date
    WHERE 1 = 0), /* ============================================================================*/ WithInventory AS
    ...
    (SELECT f.*, CASE WHEN Is_BegBal_Row = 1 THEN 0 WHEN Suppress_Stale = 1 THEN 1 WHEN Suppress_Fence = 1 THEN 1 ELSE 0 END AS Is_Suppressed, 
                 ...
                 Inventory_Qty_Available < Remaining THEN 'PARTIAL: Demand Netted' ELSE 'VALID DEMAND' END AS Suppression_Status, CASE WHEN Is_BegBal_Row = 1 THEN NULL WHEN (Suppress_Stale = 1 OR
...
FROM   Deltas d), /* ============================================================================*/ ETB_WC_INV AS
    ...
    ORDER BY Seq ROWS UNBOUNDED PRECEDING) AS decimal(18, 6)) AS Adjusted_Running_Balance, MRP_IssueDate, WCID_From_MO, WC_Site_Validation, Issued, Remaining, Net_Demand, Inventory_Qty_Available, Is_Suppressed, Suppression_Status, Remaining_After_Suppression, 
Has_Issued, IssueDate_Mismatch, Early_Issue_Flag, Last_Cycle_Count_Date, Days_Since_Last_Cycle_Count, Cycle_Count_Status, Status_Explanation, VendorItem, PRIME_VNDR, Vendor_Data_Source, PURCHASING_LT, PLANNING_LT, ORDER_POINT_QTY, SAFETY_STOCK, FG, 
[FG Desc], STSDESCR, MRPTYPE, Unified_Value
...
FROM   DemandRowsOnly d), /* ============================================================================*/ Stockout_Detection AS
    ...
    /* ============================================================================*/ SELECT ITEMNMBR, ItemDescription, UOM, ORDERNUMBER, Construct, DUEDATE, [Expiry Dates], [Date + Expiry], BEG_BAL, Deductions, Expiry, [PO's], Running_Balance, 
                Adjusted_Running_Balance, MRP_IssueDate, WCID_From_MO, WC_Site_Validation, Issued, Original_Required, Net_Demand, Inventory_Qty_Available, Suppression_Status, Last_Cycle_Count_Date, /* Issue 3*/ Days_Since_Last_Cycle_Count, /* Issue 3*/ Cycle_Count_Status, 
                /* Issue 3*/ Status_Explanation, /* Issue 3: CycleCount stub explanation*/ VendorItem, PRIME_VNDR, Vendor_Data_Source, /* Issue 4*/ PURCHASING_LT, PLANNING_LT, ORDER_POINT_QTY, SAFETY_STOCK, FG, [FG Desc], STSDESCR, MRPTYPE, Unified_Value, 
