...
dbo.ETB_PAB_MO p
WHERE
p.STSDESCR <> ‘Partially Received’ AND
p.STSDESCR <> ‘SCRAP’ AND
LTRIM(RTRIM(p.ITEMNMBR)) NOT LIKE ‘60.%’ AND
LTRIM(RTRIM(p.ITEMNMBR)) NOT LIKE ‘70.%’
),
m_norm AS (
SELECT
m.*,
...
ELSE CONCAT(Core.ITEMNMBR, ’ - ’, Core.[Date + Expiry], ’ - ‘, ml.Required_Qty - ml.Total_Issued)
