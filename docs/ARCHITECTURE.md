...
```
               ...
               ▼                ▼                     ▼
  ┌─────────────────┐  ┌───────────────────┐  ┌─────────────────────────┐
  │ ETB_STOCKOUTS   │  │  ETB_STOCKOUTS    │  │ ETB_PROGRAM_WEIGHTS     │
  │ (View 10 —      │  │  (View 10 —       │  │ ETB_WEIGHTED_DEMAND     │
  │   180d Stockout │  │   180d Stockout   │  │ ETB_WEIGHTED_SUMMARY    │
  │   Aggregation)  │  │   Aggregation)    │  │ (View 11 — Weighted     │
  └─────────────────┘  └───────────────────┘  │  Universe)              │
                                                └─────────────────────────┘
```

**Note**: Views 4 and 5 re-inline the full logic of Views 1–3 as CTEs for
performance (no view-on-view chaining in the hot path). Views 10 and 11 
...
6. **ETB_STOCKOUTS** (View 10): Aggregates stockout risk across 180-day horizon with program flags and action counts
...
```
sql/           ← SINGLE SOURCE OF TRUTH for all 7 active views/objects (Views 1–5, 10–11)
